...
public class SAXutils {
    ...
    public static double MinDistPAAToiSAX(short[] saxVals, float[] timeSeries) throws Exception {


        double minDist = 0.0;
        int wordLen = saxVals.length;





        double[] PAA = GetPAA(timeSeries, wordLen);

        int segmentSize = timeSeries.length / wordLen;


        for (int i = 0; i < wordLen; i++)
        {
            // PAA[offset]
            short card = 512 ;// card for this value
            double tVal = PAA[i];
            double ptDist = 0.0;
            ...
            if (saxVals[i] == 0)
            {
                double lrgr = breakpoints[saxVals[i]];
                if (tVal > lrgr)
                    ptDist = Math.pow((tVal - lrgr), 2);
            }
            //  highest card(val)
            else if (saxVals[i] == card - 1)
            {
                double smlr = breakpoints[saxVals[i] - 1];
                if (tVal < smlr)
                    ptDist = Math.pow((tVal - smlr), 2);
            }
            // check if pt falls on either side of breakpt region
            else
            {
                double smlr = breakpoints[saxVals[i] - 1];
                double lrgr = breakpoints[saxVals[i]];
